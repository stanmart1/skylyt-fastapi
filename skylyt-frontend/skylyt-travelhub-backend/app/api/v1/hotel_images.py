...
async def upload_hotel_images(
    ...
    for file in files:
        ...
        max_order = db.query(HotelImage).filter(HotelImage.hotel_id == hotel_id).count()
...
async def upload_hotel_image_from_url(
    hotel_id: str = Form(...),
    image_url: str = Form(...),
    current_user = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Upload hotel image from URL"""
    if not (current_user.is_admin() or current_user.is_superadmin()):
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    try:
        ...
        return {"message": "Image uploaded successfully", "image": {"id": hotel_image.id, "url": hotel_image.image_url}}
...
def get_hotel_images(
    ...
    images = db.query(HotelImage).filter(HotelImage.hotel_id == hotel_id)\
...
def set_cover_image(
    ...
    # Remove cover status from other images of the same hotel
    db.query(HotelImage).filter(HotelImage.hotel_id == image.hotel_id)\
...
def delete_hotel_image(
    ...
    try:
        # Validate the image URL path
        if not image.image_url.startswith('/uploads/hotels/'):
        ...
        # Log error but don't fail the deletion of DB record
        pass
...
def reorder_hotel_images(
    ...
    db.query(HotelImage).filter(HotelImage.id.in_(image_ids)).update({
        HotelImage.display_order: case(order_mapping, value=HotelImage.id)
